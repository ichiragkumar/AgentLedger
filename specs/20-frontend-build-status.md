...
2. ~~**Dashboard fetchers → live plane**~~ DONE (this block): new `app/api/_lib/proxy-mgmt.ts` (base resolution, admin-actor + optional bearer headers, fail-open 2.5s fetch). Keys + onboarding-key routes are proxy-first with PG fallback (Go and dashboard share `virtual_keys`; sha256/hash algo identical; revoke/grace semantics compatible). Budgets: PG-first durable write + best-effort Go mirror (`proxySynced:false` when mirror fails; Go IDs are `level:key:window`). Alerts POST now sends admin actor headers (was 403-prone) with 400/403 passthrough. Proven live: dashboard-issue → Go list → data-plane accept → dashboard-rotate (grace) → dashboard-revoke; dashboard budget create → Go has it → raise mirrored $50→$100 → delete mirrored; alerts POST → `alert-1` in Go registry. Test rows purged. `.env.example` gains `PROXY_MGMT_BASE/PROXY_MGMT_TOKEN/MGMT_TOKEN`.
