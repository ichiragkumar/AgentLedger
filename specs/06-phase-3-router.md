...
- Ship-track 2026-09-20 (routing end-to-end): Go gap closed with `internal/router/analytics.go` (Distribution shares, SummarizeSavings canonical formula, SummarizeEscalation vs 10% budget) + `analytics_test.go`; package green at 89.6% coverage. Dashboard LIVE: `app/api/routing/tiers` (GET/PUT, spec-17 tier values + live registry $/in/out, 16-model dropdown), `rules` (CRUD, wildcard semantics mirror Engine.Matches), `fallback` (GET/PUT, dedupe order), `distribution` (live 7d per-model + daily + savings vs frontier-tier baseline; quality/escalation honest stubs) — all zero-state safe, tables created idempotently in-route (`routing_tiers/rules/fallback`), writes return `proxySynced:false`. `lib/hooks/use-routing.ts` (fetchers in-file; api-ext untouched). `(app)/routing` rewritten to live hook: ACTIVE banner with live avg %, saved-$ headline (.mono), tier cards, 7d stacked bars + RoutingPanel, YAML editor with highlight + hot-reload toasts, drag + ↑/↓ fallback reorder, skeletons/empty/error states, keyboard + mobile. Verified: `gofmt -l` clean, `go vet` + `go test ./internal/router/...` green, `npx tsc --noEmit` clean, curl all routes (200s, 400/404 paths, CRUD round-trip, test rows cleaned). REQUIRED_ENV: none. Wiring: root `/WIRING.md` created (proxy middleware + config reload + optional /v1/routing/distribution). Next: coordinator applies WIRING (proxy consumes dashboard config); judge/guard export to lift quality/escalation stubs.
