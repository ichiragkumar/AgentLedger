...
1. `gofmt -l` clean on touched dirs.
2. `go vet` + `go test` green for owned packages (target >80% coverage on new code).
3. Frontend: `npx tsc --noEmit` clean if touching `dashboard/`.
4. Return: files created/modified, test output, REQUIRED_ENV additions, wiring notes.
