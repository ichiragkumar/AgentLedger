...
export default function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    const res = await signIn("dev-login", { email, password, redirect: false, callbackUrl: "/overview" });
    if (res?.error) {
      setError("Invalid email or password (dev stub — check .env.local).");
      return;
    }
    window.location.href = res?.url ?? "/overview";
  };
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-sm flex-col justify-center gap-5 px-6">
      ...
      <form onSubmit={submit} className="flex flex-col gap-3">
        ...
        <button type="submit" className="rounded-lg bg-green-700 px-4 py-2 text-sm font-medium text-white hover:bg-green-800">
          Sign in
        </button>
      </form>
